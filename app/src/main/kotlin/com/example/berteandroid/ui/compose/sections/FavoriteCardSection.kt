...
@Composable
fun FavoriteCollectionCardSection(data: List<ElementData>) {
    LazyHorizontalGrid(
    ...
    ) { items(data) { FavoriteCollectionCard(it) } }
}
...
fun FavoriteCollectionCard(element: ElementData, modifier: Modifier = Modifier) {
    Surface(
        ...
        Row(
            ...
            modifier = Modifier.width(182.dp).background(MaterialTheme.colorScheme.secondary)
        ) {
            Image(
